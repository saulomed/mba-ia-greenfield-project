...
// Remapped keys drop the original method declarations, so asserting on
// `mock.method` is not reported as an unbound method reference.
type MethodMocks<T, K extends keyof T> = {
  [P in K as P & string]: jest.MockedFunction<
    Extract<T[P], (...args: any[]) => any>
  >;
};
type UsersServiceMock = MethodMocks<
  UsersService,
  'findByEmail' | 'findByEmailWithChannel' | 'createUserWithChannel' | 'save'
>;
type MailServiceMock = MethodMocks<
  MailService,
  'sendConfirmationEmail' | 'sendPasswordResetEmail'
>;
type VerificationTokenRepositoryMock = MethodMocks<
  Repository<VerificationToken>,
  'create' | 'save' | 'findOne' | 'createQueryBuilder'
>;
type RefreshTokenRepositoryMock = MethodMocks<
