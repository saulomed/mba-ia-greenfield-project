...
issue_count: 0
sources_mtime:
  docs/phases/phase-03-videos/context.md: "2026-09-13T17:04:35Z"
...
- **OQ-1** _(resolved_by phase-03-videos/TD-01)_ — TD-01 decidida: A (S3 Multipart com URLs pré-assinadas por parte).
- **OQ-2** _(resolved_by phase-03-videos/TD-02)_ — TD-02 decidida: A (MinIO no Compose + `@aws-sdk/client-s3` v3).
- **OQ-3** _(resolved_by phase-03-videos/TD-03)_ — TD-03 decidida: A (BullMQ + Redis via `@nestjs/bullmq`).
- **OQ-4** _(resolved_by phase-03-videos/TD-04)_ — TD-04 decidida: A (container separado, mesmo codebase).
- **OQ-5** _(resolved_by phase-03-videos/TD-05)_ — TD-05 decidida: A (FFmpeg do sistema + wrapper sobre `spawn`).
- **OQ-6** _(resolved_by phase-03-videos/TD-06)_ — TD-06 decidida: A (endpoint explícito de conclusão na API).
- **OQ-7** _(resolved_by phase-03-videos/TD-07)_ — TD-07 decidida: B (base62 aleatório em `public_id`).
- **OQ-8** _(resolved_by phase-03-videos/TD-08)_ — TD-08 decidida: B (MP4 H.264/AAC normalizado com `+faststart`).
- **OQ-9** _(resolved_by phase-03-videos/TD-09)_ — TD-09 decidida: A (URLs pré-assinadas de curta duração).
- **OQ-10** _(resolved_by phase-03-videos/TD-10)_ — TD-10 decidida: A (lifecycle do bucket + job de rascunhos).
- **OQ-11** _(resolved_by phase-03-videos/TD-11)_ — TD-11 decidida: C (prefixo público `thumbnails/`, chaves imprevisíveis e imutáveis).
- **OQ-12** _(resolved_by phase-03-videos/TD-12)_ — TD-12 decidida: B (fixtures geradas sob demanda com FFmpeg `lavfi` no `globalSetup` do Jest, stream sintético em múltiplos de 5 MB para o multipart e limite de upload reduzido por configuração no ambiente de teste).
